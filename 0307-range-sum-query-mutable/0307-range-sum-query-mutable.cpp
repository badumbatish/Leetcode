...
public:
    NumArray(vector<int>& nums) : nums(nums){
        block_size = std::ceil(std::sqrt(nums.size()));
    ...
    int sumRange(int left, int right) {
        ...
        if (start_block == end_block) return std::accumulate(nums.begin() + left, nums.begin() + right + 1, 0);

        int sum = 0;
        for (int k = left; k < (start_block + 1) * block_size; k++) sum += nums[k];
        for (int k = (start_block + 1); k < end_block; k++) sum += blocks[k];
